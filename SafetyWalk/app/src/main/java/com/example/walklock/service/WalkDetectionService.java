...
import com.example.walklock.receiver.LockScreenReceiver;
import com.example.walklock.ui.LockScreenActivity;
...
//    private ActivityRecognitionClient activityRecognitionClient;
    ...
    //设置阈值，避免利用开 关屏，来规避检测。因为每次关闭检测后再次启动检测重新开始计算。
    //利用重新检测的空档进行规避检测。情景：锁屏，开屏，重新检测。
//    private final int LOCK_INTER = 20 * 1000; // 亮屏和锁屏之间超过30s进行关闭.避免规避传感器检测。
//    private long preUnlockTime = 0;

//    private int niceTry = 0;
...
//如果想在应用被移除时执行一些清理工作，应该放在 onTaskRemoved() 中，而不是 onDestroy() 中。因为只有 onTaskRemoved() 能保证被调用。
    ...
    public void onDestroy() {
        ...
        // 非正常停止时重启
//        if (!isNormalStop) {
//            Log.d(TAG, "onDestroy: restarting servcice");
//            Intent restartServiceIntent = new Intent(getApplicationContext(), this.getClass());
//            restartServiceIntent.setPackage(getPackageName());
//            PendingIntent restartServicePendingIntent = PendingIntent.getService(
//                    getApplicationContext(), 1, restartServiceIntent,
